...
export class SimpleAdvancedCalculator extends Calculator {
  private advancedMode: 'basic' | 'scientific' | 'matrix' | 'statistics' | 'units' | 'complex' = 'basic';
  ...
  private async performMatrixOperation(operation: string): Promise<void> {
    try {
      ...
      // 获取矩阵数据
      const a11 = parseFloat((this.container.querySelector('#a11') as HTMLInputElement)?.value || '0');
      const a12 = parseFloat((this.container.querySelector('#a12') as HTMLInputElement)?.value || '0');
      const a21 = parseFloat((this.container.querySelector('#a21') as HTMLInputElement)?.value || '0');
      const a22 = parseFloat((this.container.querySelector('#a22') as HTMLInputElement)?.value || '0');
  ...
  private async performStatisticsOperation(operation: string): Promise<void> {
    try {
      ...
      const dataText = (this.container.querySelector('#stats-data') as HTMLTextAreaElement)?.value || '';
  ...
  private async performComplexOperation(operation: string): Promise<void> {
    try {
      ...
      const aReal = parseFloat((this.container.querySelector('#a-real') as HTMLInputElement)?.value || '0');
      const aImag = parseFloat((this.container.querySelector('#a-imag') as HTMLInputElement)?.value || '0');
      const bReal = parseFloat((this.container.querySelector('#b-real') as HTMLInputElement)?.value || '0');
      const bImag = parseFloat((this.container.querySelector('#b-imag') as HTMLInputElement)?.value || '0');
  ...
  private async performUnitConversion(): Promise<void> {
    try {
      ...
      const value = parseFloat((this.container.querySelector('#conv-value') as HTMLInputElement)?.value || '0');
      const fromUnit = (this.container.querySelector('#from-unit') as HTMLSelectElement)?.value || 'm';
