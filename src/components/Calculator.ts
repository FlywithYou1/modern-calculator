...
export class Calculator {
      ...
      if (invoke) {
        try {
          ...
          if (backendSettings) {
            ...
            if (typeof backendSettings.display?.decimalPlaces === 'number') mapped.precision = backendSettings.display.decimalPlaces;
            if (backendSettings.display?.angleUnit) mapped.angleUnit = backendSettings.display.angleUnit === 'degrees' || backendSettings.display?.angleUnit === 'radians' ? backendSettings.display.angleUnit : 'degrees';
            if (typeof backendSettings.general?.enableAnimations === 'boolean') mapped.enableAnimations = backendSettings.general.enableAnimations;
            if (typeof backendSettings.general?.enableHaptic === 'boolean') mapped.enableHaptics = backendSettings.general.enableHaptic;
            if (typeof backendSettings.layout?.compactMode === 'boolean') mapped.compactMode = backendSettings.layout.compactMode;
            if (typeof backendSettings.layout?.showHistory === 'boolean') mapped.showHistory = backendSettings.layout.showHistory;
            mapped.showMemory = true;
            if (typeof backendSettings.general?.enableKeyboardShortcuts === 'boolean') mapped.enableKeyboardShortcuts = backendSettings.general.enableKeyboardShortcuts;
  ...
  private setupEventListeners(): void {
    // 标题栏按钮
    this.container.querySelector('#history-btn')?.addEventListener('click', () => this.toggleSidebar('history'));
    this.container.querySelector('#settings-btn')?.addEventListener('click', () => this.toggleSidebar('settings'));
    ...
    // 内存控制按钮
    this.container.querySelector('#memory-clear')?.addEventListener('click', () => this.memoryOperation('clear'));
    this.container.querySelector('#memory-recall')?.addEventListener('click', () => this.memoryOperation('recall'));
    this.container.querySelector('#memory-store')?.addEventListener('click', () => this.memoryOperation('store'));
    this.container.querySelector('#memory-add')?.addEventListener('click', () => this.memoryOperation('add'));
  ...
  private async evaluateExpression(expression: string): Promise<string> {
    try {
      // 使用 Tauri 后端的高精度计算
      const result = await invoke<{ success: boolean; result?: string; error?: string }>('calculate', { expression });
      ...
      return evaluateExpressionSafe(expression, { angleUnit, precision: this.state.settings.precision })
    }
  }
