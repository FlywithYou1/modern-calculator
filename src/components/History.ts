...
export class History {
  ...
  private createHistoryItemHTML(item: HistoryItem): string {
    ...
    const tags = item.tags ? item.tags.map(tag => `<span class="history-tag">${tag}</span>`).join('') : '';
  ...
  private calculateStats(): HistoryStats {
    ...
    return {
      ...
      calculationsToday,
      averageExpressionLength: this.history.reduce(
        (acc, item) => acc + item.expression.length, 0
